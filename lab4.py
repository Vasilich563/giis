...
class Figure:
    ...
    def update_midpoint(self):
        ...
        sum_x = sum(dot.x_coordinate for dot in self.dots)
        sum_y = sum(dot.y_coordinate for dot in self.dots)
        sum_z = sum(dot.z_coordinate for dot in self.dots)
    ...
    def rotate(self, angle, axis):
        ...
        for dot in self.dots:
            dot.x_coordinate, dot.y_coordinate, dot.z_coordinate = np.dot(
                rotation_matrix, [dot.x_coordinate, dot.y_coordinate, dot.z_coordinate]
            )
        self.update_midpoint()
    ...
    def get_rotation_matrix(self, angle, axis):
        ...
        if axis == "x":
            ...
            # y' = y * cos - z sin
            # z' = y * sin + z cos
            return np.array([[1, 0, 0], [0, angle_cos, -angle_sin], [0, angle_sin, angle_cos]])
        elif axis == "y":
            # Y rotate
            # x' = x * cos + z * sin
            # y' = y
            # z' = -x * sin + z * cos
            return np.array([[angle_cos, 0, angle_sin], [0, 1, 0], [-angle_sin, 0, angle_cos]])
        elif axis == "z":
            # Z rotate
            # x' = x * cos - y * sin
            # y' = x * sin + y cos
            # z' = z
            return np.array([[angle_cos, -angle_sin, 0], [angle_sin, angle_cos, 0], [0, 0, 1]])

    def scale(self, scale_factor):
        for dot in self.dots:
            dot.x_coordinate = (
                                       dot.x_coordinate - self.midpoint.x_coordinate
            ) * scale_factor + self.midpoint.x_coordinate
            dot.y_coordinate = (
                                       dot.y_coordinate - self.midpoint.y_coordinate
            ) * scale_factor + self.midpoint.y_coordinate
            dot.z_coordinate = (
                                       dot.z_coordinate - self.midpoint.z_coordinate
            ) * scale_factor + self.midpoint.z_coordinate
        self.update_midpoint()
...
class GraphicsEditor3D:
    def __init__(self, root):
        ...
        self.dot_frame.configure(bg="yellow")
        self.dot_frame.grid(row=5, column=0, padx=10, pady=10)
        ...
        self.add_button.grid(row=2, column=2, columnspan=2, padx=5, pady=5)

        self.create_figure_button = tk.Button(
            self.dot_frame, text="Create Figure", command=self.create_figure
        )
        self.create_figure_button.grid(row=3, column=1, columnspan=2, padx=5, pady=5)

        self.clear_button = tk.Button(
            self.dot_frame, text="Clear", command=self.clear_all
        )
        self.clear_button.grid(row=3, column=2, columnspan=2, padx=5, pady=5)

        self.shapes_combobox = ttk.Combobox(
            self.root, values=self.shapes, state="readonly"
        )
        self.shapes_combobox.grid(row=4, column=0, padx=10, pady=10)
        ...
        if self.shapes:
            with open("figures.txt", "w") as file:
                for figure in self.shapes:
                    dots_str = " ".join(
                        f"{dot.x_coordinate},{dot.y_coordinate},{dot.z_coordinate}"
    ...
    def rotate_selected_figure_x(self, *args):
        if self.selected_figure:
            self.selected_figure.rotate(np.pi / 18., "x")
            self.update_plot()

    def rotate_selected_figure_y(self, *args):
        if self.selected_figure:
            self.selected_figure.rotate(np.pi / 18., "y")
            self.update_plot()

    def rotate_selected_figure_z(self, *args):
        if self.selected_figure:
            self.selected_figure.rotate(np.pi / 18., "z")
            self.update_plot()
    ...
    def x_changed(self, *args):
        try:
            self.preview_dot.x_coordinate = float(self.x_entry.get())
        except ValueError:
            return
    ...
    def y_changed(self, *args):
        try:
            self.preview_dot.y_coordinate = float(self.y_entry.get())
        except ValueError:
            return
    ...
    def z_changed(self, *args):
        try:
            self.preview_dot.z_coordinate = float(self.z_entry.get())
        except ValueError:
            return
    ...
    def create_figure(self):
        ...
        figure_tuple = Shapes(name=name, value=figure)
    ...
    def update_plot(self):
        ...
        for figure in self.shapes:
            ...
            x_values = [dot.x_coordinate for dot in dots]
            y_values = [dot.y_coordinate for dot in dots]
            z_values = [dot.z_coordinate for dot in dots]
            self.draw_figure(x_values, y_values, z_values)

        if self.dots:
            marked_x = [dot.x_coordinate for dot in self.dots]
            marked_y = [dot.y_coordinate for dot in self.dots]
            marked_z = [dot.z_coordinate for dot in self.dots]
            self.ax.scatter(marked_x, marked_y, marked_z, color="red")

        if self.preview_dot:
            self.ax.scatter(
                self.preview_dot.x_coordinate,
                self.preview_dot.y_coordinate,
                self.preview_dot.z_coordinate,
                color="green",
            )
        ...
        if self.selected_figure:
            ...
            for dot in self.selected_figure.dots:
                self.ax.scatter(dot.x_coordinate, dot.y_coordinate, dot.z_coordinate, color="cyan")
            self.ax.scatter(
                self.selected_figure.midpoint.x_coordinate,
                self.selected_figure.midpoint.y_coordinate,
                self.selected_figure.midpoint.z_coordinate,
                color="purple",
            )
