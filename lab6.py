...
class PolygonEditor:
    def __init__(self, master):
        ...
        self.canvas = tk.Canvas(self.master, width=800, height=500, bg="yellow")
        self.canvas.grid(row=0, column=0)
        self.combobox = Combobox(self.master, values=["Растровая развертка с упорядоченным списком ребер",
                                                      "Простой алгоритм заполнения с затравкой",
                                                      "Построчный алгоритм заполнения с затравкой"], width=45)
        ...
        self.submit = tk.Button(self.master, text="submit", command=self.algorythm)
        self.combobox.grid(row=0, column=1)

        self.combobox.current(0)
        self.submit.grid(row=0, column=2)

        self.clear = tk.Button(self.master, text="clear", command=self.clear_canvas)
    ...
    def algorythm(self):
        ...
        if choice == "Растровая развертка с упорядоченным списком ребер":
            self.set_fill_algorithm_edge()
        if choice == "Простой алгоритм заполнения с затравкой":
            self.set_fill_algorithm_edge()
        if choice == "Построчный алгоритм заполнения с затравкой":
            self.set_fill_algorithm_scanline()
        ...
        for y in range(y_list[0], y_list[-1] + 1):
            for edge in self.edges:
                if edge[0][1] <= y < edge[1][1] or edge[1][1] <= y < edge[0][1]:
                    x_intersection = int(
                        edge[0][0] + (y - edge[0][1]) * (edge[1][0] - edge[0][0]) / (edge[1][1] - edge[0][1]))
